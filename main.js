...
class IconButton extends HTMLElement {
    ...
    default() {
        const homeContainer = document.querySelector('icon-button[icon="home"]');
    ...
    colormenu() {
        ...
        const homebildschirm = document.body.querySelector('homebildschirm-container');
        if(homebildschirm) {
            homebildschirm.remove();
        }
    }
...
class HomebildschirmContainer extends HTMLElement {
    constructor() {
        ...
        const homescreenContent = document
            ...
            .content
            .cloneNode(true);
        this.attachShadow({ mode: 'open' }).appendChild(homescreenContent);
    }
}
...
class ColorContainer extends HTMLElement {
    ...
    setFarbe(f) {
        document.documentElement.style.setProperty("--farbe", f);
        fetch('/farbe', {
            method: "POST",
...
class WerkzeugdatenContainer extends HTMLElement {
    constructor() {
        super();
        const werkzeugContent = document.getElementById('werkzeugdaten-template').content.cloneNode(true);
        this.attachShadow({ mode: 'open' }).appendChild(werkzeugContent);
        this.updateData();
    }
